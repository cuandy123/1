...
class action extends app
{
	...
	private function output()
	{
		...
		foreach($questionrows as $p)
		{
			...
			$r[] = array('questiontype' => $p['qrtype'],'question' => iconv("UTF-8","GBK",html_entity_decode($p['qrquestion'])),'questionselect' => iconv("UTF-8","GBK",html_entity_decode($p['qrselect'])),'questionselectnumber' => iconv("UTF-8","GBK",$p['qrselectnumber']),'questionanswer' => iconv("UTF-8","GBK",html_entity_decode($p['qranswer'])),'questiondescribe' => iconv("UTF-8","GBK",html_entity_decode($p['qrdescribe'])),'knowsid' => iconv("UTF-8","GBK",html_entity_decode($knows['knows'])),'questionlevel' => $p['qrlevel']);
			$qtp = $this->exam->getSimpleQuestionListByArgs(array(array("AND","questionparent = :qrid",'qrid',$p['qrid']),array("AND","questionstatus = 1")));
			foreach($qtp as $qt)
			{
				$r[] = array('questiontype' => $p['qrtype'],'question' => iconv("UTF-8","GBK",html_entity_decode($qt['question'])),'questionselect' => iconv("UTF-8","GBK",html_entity_decode($qt['questionselect'])),'questionselectnumber' => iconv("UTF-8","GBK",$qt['questionselectnumber']),'questionanswer' => iconv("UTF-8","GBK",html_entity_decode($qt['questionanswer'])),'questiondescribe' => iconv("UTF-8","GBK",html_entity_decode($qt['questiondescribe'])),'knowsid' => iconv("UTF-8","GBK",html_entity_decode($knows['knows'])),'questionlevel' => $p['questionlevel']);
			}
		}
